...
void assert_equal(char** expected, char** actual, const char* message,
    ...
    for (int i = 0; i < len; i++) {
        if (!strcmp(expected[i], actual[i])) {
            ...
            printf(
                "[FAILED Test %d: %s]: At index: %d, expected string: %s, "
                "actual "
                "string: %s\n",
...
void test_reverse() {
    ...
    // 2. declare a char* array which is the expected result of the reversed
    // array (expected)
    ...
    // for the test, and number of elements to be reversed
}
...
void print_test_summary() {
    printf(
        "Passed %d tests, Failed %d tests | %f%%\n", num_correct, num_incorrect,
